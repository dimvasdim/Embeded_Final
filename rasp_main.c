...
#include "client_rasp.h"
#include "server_rasp.h"
...
int main(int argc, char **argv)
{
  ...
  pthread_mutex_init(&rasp_mutex, NULL);
  ...
  pthread_mutex_destroy(&rasp_mutex);
  pthread_exit(NULL);
  return 1;
}
...
char *create_message(char **messages, unsigned int *nodes, int num_of_mess, int num_of_nodes)
{
  ...
  snprintf(temp, MESSAGE_LENGTH,"%u_%u_%" PRIu64 "_%s", RASP_AEM, nodes[rand_node], timestamp, messages[rand_message]);
  strcpy(str, temp);
